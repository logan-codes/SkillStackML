...
def fetch_with_browser(url: str) -> dict:
    ...
    """
    logger.info(f"[browser_fetcher] launching Chromium for: {url}")
    ...
    with sync_playwright() as pw:
        ...
        try:
            logger.info(f"[browser_fetcher] navigating...")
            ...
            logger.info(f"[browser_fetcher] page loaded — taking screenshot")

            # Full-page screenshot
            screenshot_bytes = page.screenshot(full_page=True, type="png")
            logger.info(f"[browser_fetcher] screenshot size = {len(screenshot_bytes)} bytes")

            # Extract DOM text
            dom_text = _extract_dom_text(page)
            logger.info(f"[browser_fetcher] DOM text = {len(dom_text)} chars")

            # OCR the screenshot
            ocr_text = _ocr_screenshot(screenshot_bytes)
            logger.info(f"[browser_fetcher] OCR text = {len(ocr_text)} chars")
...
def _extract_dom_text(page) -> str:
    ...
    try:
        ...
        logger.info(f"[browser_fetcher] DOM extract error: {e}")
    return ""
...
def _ocr_screenshot(png_bytes: bytes) -> str:
    ...
    try:
        ...
        logger.info(f"[browser_fetcher] OCR error: {e}")
        return ""
    finally:
